...
vi.mock("@/lib/db", () => ({
  prisma: {
    terminalCaisse: { findFirst: vi.fn(), findUnique: vi.fn(), findMany: vi.fn() },
...
// ─── GET /api/terminaux (liste des caisses) ────────────

describe("GET /api/terminaux", () => {
  let GET: (req: Request) => Promise<Response>;

  beforeEach(async () => {
    vi.clearAllMocks();
    GET = (await import("@/app/api/terminaux/route")).GET;
    (prisma.terminalCaisse.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([fakeCaisse]);
  });

  it("returns 401 if not authenticated", async () => {
    (auth as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await GET(new Request("http://localhost/api/terminaux"));
...
// ─── GET /api/terminaux/[id]/soldes ────────────────────

describe("GET /api/terminaux/[id]/soldes", () => {
  let GET: (req: Request, ctx: { params: Promise<{ id: string }> }) => Promise<Response>;

  beforeEach(async () => {
    vi.clearAllMocks();
    GET = (await import("@/app/api/terminaux/[id]/soldes/route")).GET;
  ...
  it("returns 401 if not authenticated", async () => {
    (auth as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await GET(new Request("http://localhost/api/terminaux/caisse-1/soldes"), { params });
    expect(res.status).toBe(401);
  });
  ...
  it("returns 404 if caisse not found", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/terminaux/unknown/soldes"), { params });
    expect(res.status).toBe(404);
  });

  it("returns 500 on DB error", async () => {
    mockSession("ADMIN");
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("DB"));
    const res = await GET(new Request("http://localhost/api/terminaux/caisse-1/soldes"), { params });
    expect(res.status).toBe(500);
  });
});

// ─── GET /api/terminaux/[id]/mouvements ────────────────

describe("GET /api/terminaux/[id]/mouvements", () => {
  let GET: (req: Request, ctx: { params: Promise<{ id: string }> }) => Promise<Response>;
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    GET = (await import("@/app/api/terminaux/[id]/mouvements/route")).GET;
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue(fakeCaisse);
  ...
  it("returns 401 if not authenticated", async () => {
    (auth as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await GET(new Request("http://localhost/api/terminaux/caisse-1/mouvements"), { params });
    expect(res.status).toBe(401);
  });
  ...
  it("returns 404 if caisse not found", async () => {
    mockSession("ADMIN");
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await GET(new Request("http://localhost/api/terminaux/unknown/mouvements"), { params });
    expect(res.status).toBe(404);
  });
});

// ─── POST /api/terminaux/[id]/mouvements ───────────────

describe("POST /api/terminaux/[id]/mouvements", () => {
  ...
  it("MANAGER creates APPORT on caisse", async () => {
    ...
    expect(createMovement).toHaveBeenCalledWith(
      expect.objectContaining({ type: "APPORT", terminalId: "caisse-1", montant: 5000 }),
    );
  });

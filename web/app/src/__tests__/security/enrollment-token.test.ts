...
describe("issueEnrollmentToken", () => {
  it("émet un token clair de 64 hex et ne persiste que le hash", async () => {
    ...
    const res = await issueEnrollmentToken({ terminalId: "c1", ttlMinutes: 60 });
    expect(res.token).toMatch(/^[0-9a-f]{64}$/);
    expect(res.id).toBe("et-1");
    expect(res.expiresAt).toBeInstanceOf(Date);
    const arg = create.mock.calls[0][0].data;
    expect(arg.tokenHash).toBe(hashToken(res.token));
    expect(arg.terminalId).toBe("c1");
  });
});

describe("consumeEnrollmentToken", () => {
  it("token valide non consommé non expiré → consommé (updateMany 1) + terminalId", async () => {
    const token = "a".repeat(64);
    findUnique.mockResolvedValue({
      id: "et-1", terminalId: "c1", consumedAt: null,
      expiresAt: new Date(Date.now() + 60_000),
    });
    updateMany.mockResolvedValue({ count: 1 });
    const res = await consumeEnrollmentToken(token);
    expect(res).toEqual({ valid: true, terminalId: "c1", tokenId: "et-1" });
  ...
  it("token inconnu → invalide", async () => {
    ...
    expect(await consumeEnrollmentToken("b".repeat(64))).toEqual({ valid: false, terminalId: null, tokenId: null });
  });

  it("token déjà consommé → invalide", async () => {
    findUnique.mockResolvedValue({ id: "et-1", terminalId: "c1", consumedAt: new Date(), expiresAt: new Date(Date.now() + 60_000) });
    expect((await consumeEnrollmentToken("a".repeat(64))).valid).toBe(false);
  });

  it("token expiré → invalide", async () => {
    findUnique.mockResolvedValue({ id: "et-1", terminalId: "c1", consumedAt: null, expiresAt: new Date(Date.now() - 1) });
    expect((await consumeEnrollmentToken("a".repeat(64))).valid).toBe(false);
  });

  it("course : updateMany count 0 (consommé entre-temps) → invalide", async () => {
    findUnique.mockResolvedValue({ id: "et-1", terminalId: "c1", consumedAt: null, expiresAt: new Date(Date.now() + 60_000) });

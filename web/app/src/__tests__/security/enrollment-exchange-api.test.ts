...
describe("POST /api/enrollment/exchange", () => {
  ...
  it("code valide + nom → 200, consomme, renomme, émet store token", async () => {
    consumeEnrollmentToken.mockResolvedValue({ valid: true, terminalId: "c1", tokenId: "et-1" });
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", active: true, code: "P1", nom: "Ancien" });
    (prisma.terminalCaisse.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", code: "P1", nom: "Caisse Entrée" });
    ...
    expect(body.data.terminalId).toBe("c1");
    expect(body.data.codePoste).toBe("P1");
    expect(body.data.nom).toBe("Caisse Entrée");
    expect(prisma.terminalCaisse.update).toHaveBeenCalledWith({ where: { id: "c1" }, data: { nom: "Caisse Entrée" }, select: { id: true, code: true, nom: true } });
  });

  it("nom vide → ne renomme pas, garde le nom existant", async () => {
    consumeEnrollmentToken.mockResolvedValue({ valid: true, terminalId: "c1", tokenId: "et-1" });
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", active: true, code: "P1", nom: "Existant" });
    issueStoreToken.mockResolvedValue({ token: "s".repeat(64), id: "st-1" });

    const res = await POST(req({ token: "a".repeat(64) }));
    expect(res.status).toBe(200);
    expect((await res.json()).data.nom).toBe("Existant");
    expect(prisma.terminalCaisse.update).not.toHaveBeenCalled();
  });

  it("code invalide/expiré/consommé → 401, pas de store token", async () => {
    consumeEnrollmentToken.mockResolvedValue({ valid: false, terminalId: null, tokenId: null });
    const res = await POST(req({ token: "a".repeat(64) }));
    expect(res.status).toBe(401);
    expect(issueStoreToken).not.toHaveBeenCalled();
  });

  it("terminal inactif → 422", async () => {
    consumeEnrollmentToken.mockResolvedValue({ valid: true, terminalId: "c1", tokenId: "et-1" });
    (prisma.terminalCaisse.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", active: false, code: "P1", nom: "X" });
    const res = await POST(req({ token: "a".repeat(64) }));

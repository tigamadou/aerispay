...
| **KPI-04** | Encaissement espèces (jour) | **Somme** des montants des `Paiement` avec `method = ESPECES` rattachés à des ventes `COMPLETED` du jour J. | Monnaie |
| **KPI-05** | Autres encaissements (jour) | **Somme** des paiements **non** `ESPECES` (mobile money) des ventes `COMPLETED` du jour J. | Monnaie |
...
| **Lien d’action principal** | Bouton ou card vers **`/comptoir`** (ouverture / reprise de vente). |
